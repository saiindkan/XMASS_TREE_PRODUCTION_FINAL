...
export async function POST(request: NextRequest) {
  try {
    const { payment, customerInfo, total } = await request.json()

    if (!payment || !customerInfo || !total) {
    ...
    const paymentIntent = await stripe.paymentIntents.create({
      ...
      description: `Apple Pay - ${customerInfo.name}`,
    ...
    const orderData = {
      ...
      notes: 'Apple Pay payment completed'
    }
